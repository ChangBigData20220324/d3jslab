const parseNA1 = string => (string === 'NA' ? undefined : string);
//日期處理
const parseDate1 = string => d3.timeParse('%Y-%m-%d')(string);
//Data selection
function type(d) {
    const date = parseDate1(d.release_date);
    return {
        budget: +d.budget,
        genre: parseNA1(d.genre),
        genres: JSON.parse(d.genres).map(d => d.name),
        homepage: parseNA1(d.homepage),
        id: +d.id,
        imdb_id: parseNA1(d.imdb_id),
        original_language: parseNA1(d.original_language),
        overview: parseNA1(d.overview),
        popularity: +d.popularity,
        poster_path: parseNA1(d.poster_path),
        production_countries: JSON.parse(d.production_countries),
        release_date: date,
        release_year: date.getFullYear(),
        revenue: +d.revenue,
        runtime: +d.runtime,
        tagline: parseNA1(d.tagline),
        title: parseNA1(d.title),
